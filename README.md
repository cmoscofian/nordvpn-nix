...
I'll try and keep this up to date, but if you need to do it yourself on line 28 you'll need to change the version.  You can find the latest version of the deb files [here](https://repo.nordvpn.com/deb/nordvpn/debian/pool/main/n/nordvpn/) get the version number i.e. 4.1.2 and replace the version

Make the hash in line 35 an empty string `""` and then do `sudo nixos-rebuild switch` it will fail due to the hash being in correct, paste the correct hash into the string and then re-run `sudo nixos-rebuild switch` and it should build
